...
import styles from "./TajikistanPrivate.module.scss";
import mainImg from "../../../assets/Countries/tajikistan.jpg"; // 🖼️ use your actual image path

const STRAPI_BASE = "https://brilliant-passion-7d3870e44b.strapiapp.com";

export default function TajikistanPrivateTours() {
  ...
  // ✅ fetch tours (Tajikistan, only Private tours)
  useEffect(() => {
    const url = strapiLocale
      ? `${STRAPI_BASE}/api/asian-tours?locale=${strapiLocale}&filters[location][$eq]=Tajikistan`
      : `${STRAPI_BASE}/api/asian-tours?filters[location][$eq]=Tajikistan`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        setTours(
          normalized.filter(
            (t) =>
              (t.location || "").toLowerCase().includes("tajikistan") &&
  ...
  tours.forEach((t) => {
    ...
    if (ttype.includes("adventure") || ttype.includes("mountain"))
      categories.Adventure.push(t);
    if (ttype.includes("eco") || ttype.includes("nature"))
      categories.Eco.push(t);
    if (ttype.includes("city")) categories.City.push(t);
    if (ttype.includes("trek") || ttype.includes("hiking"))
      categories.Trekking.push(t);
      ...
      <div className={styles.container}>
        <div className={styles.content}>
          ...
          <div className={styles.tours}>
            ...
            <h2 className={styles.sectionTitle}>Tajikistan Private Tours</h2>
            ...
            <p className={styles.description}>
              Discover Tajikistan privately with personalized journeys across
              breathtaking Pamir Mountains, ancient Silk Road towns, and rich
              cultural heritage. Enjoy comfort and freedom on every route.
            </p>
          </div>

          {/* RIGHT */}
          <aside className={styles.sidebar}>
            <h3>Tajikistan Private Tours</h3>
