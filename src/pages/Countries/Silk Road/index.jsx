...
export default function SilkRoadTours() {
  ...
  // fetch tours (Silk Road only)
  useEffect(() => {
    const url = strapiLocale
      ? `${STRAPI_BASE}/api/asian-tours?locale=${strapiLocale}&filters[location][$eq]=Silk Road`
      : `${STRAPI_BASE}/api/asian-tours?filters[location][$eq]=Silk Road`;
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        setTours(
          normalized.filter(
            (t) =>
              (t.location || "").toLowerCase().includes("silk") &&
  ...
  tours.forEach((t) => {
    ...
    if (ttype.includes("cultural") || ttype.includes("heritage"))
      pushIf("Cultural");
    if (ttype.includes("food") || ttype.includes("culinary"))
      pushIf("Gastronomy");
      ...
      <div className={styles.container}>
        <div className={styles.content}>
          <div className={styles.tours}>
            ...
            <h2 className={styles.sectionTitle}>Silk Road Tours</h2>
            ...
            <p className={styles.description}>
              Step into history and explore the legendary Silk Road — a route
              connecting East and West through Uzbekistan, Kyrgyzstan,
              Tajikistan, Turkmenistan, and Kazakhstan. Discover timeless
              architecture, colorful bazaars, and the cultures that shaped
