...
import styles from "./ReligiousTours.module.scss";
...
export default function ReligiousTours() {
  ...
  // ✅ Filter tours that have "Religious" in tour_type
  const religiousTours = tours.filter((tour) =>
    tour.tour_type?.toLowerCase().includes("religious")
  );

  // ✅ Separate into group and private religious tours
  const groupReligiousTours = religiousTours.filter((tour) =>
    tour.tour_type?.toLowerCase().includes("group")
  );

  const privateReligiousTours = religiousTours.filter((tour) =>
        ...
        >
          ...
          <div className={styles.cardBody}>
            <div className={styles.tags}>
              <span>RELIGIOUS EXPERIENCE</span>
              <span>Faith, Culture & Heritage</span>
            </div>
    ...
    <div className={styles.wrapper}>
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          ...
          <p>
            Explore sacred sites and spiritual landmarks across Asia. Immerse
            yourself in cultural traditions and faith-based experiences that
          ...
          <div className={styles.resultsCount}>
            {religiousTours.length} Trips
          </div>
        </div>

        {/* Group Religious Tours */}
        {groupReligiousTours.length > 0 && (
          <>
            <h2 className={styles.sectionTitle}>Group Religious Tours</h2>
            <div className={styles.toursGrid}>
              {renderTours(groupReligiousTours)}
            </div>
          </>
        )}

        {/* Private Religious Tours */}
        {privateReligiousTours.length > 0 && (
          <>
            <h2 className={styles.sectionTitle}>Private Religious Tours</h2>
            <div className={styles.toursGrid}>
              {renderTours(privateReligiousTours)}
            </div>
          </>
        )}
      </div>
